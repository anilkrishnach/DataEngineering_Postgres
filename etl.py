...
def process_log_file(cur, filepath):
    ...
    user_df = user_df.drop_duplicates(subset=['userId'], keep='first').dropna()#To drop duplicates and null valued rows
    ...
    for i,row in df.iterrows():        
        ...
        songplay_id = str(row.userId)+"_"+str(row.ts) #At any point of time, a user can request only one song. 
        songplay_data = (songplay_id, dt.datetime.fromtimestamp(row.ts/1000.0), row.userId, row.level, songid, artistid, row.sessionId, row.location, row.userAgent)
